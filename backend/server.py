...
def split_manuscript(raw_text: str) -> tuple:
    ...
    # ── Step 2: Batch consecutive short chapters together ────────────────────
    batched = []
    i = 0
    while i < len(raw_chapters):
        ...
        if wc < BATCH_THRESHOLD and i + 1 < len(raw_chapters):
            # Try to merge with next chapter(s) while combined stays under SPLIT_THRESHOLD
            ...
            while j < len(raw_chapters):
                ...
                if len(combined_text.split()) + next_wc <= SPLIT_THRESHOLD:
                    ...
                    if next_wc >= BATCH_THRESHOLD:
                        break  # stop batching once we've added a full-size chapter
                else:
                    break
    ...
    # ── Step 3: Scene breaks + sub-split chapters >5000 words ────────────────
    scene_break_pattern = re.compile(
        r'\n[ \t]*(?:\*{2,}|\-{2,}|#{2,}|~{2,}|_{2,}|\.{3,})[ \t]*\n|\n{3,}',
        re.MULTILINE
    )

    def split_on_scenes(title: str, text: str) -> List[Dict]:
        parts = scene_break_pattern.split(text)
        result = []
    ...
    def sub_split(title: str, text: str) -> List[Dict]:
        """Sub-split only if text exceeds SPLIT_THRESHOLD words."""
        words = text.split()
        ...
        for para in paragraphs:
            ...
            if current_words + pw > TARGET_WORDS_PER_SECTION and current_paras:
                part_num += 1
                chunks.append({"title": title if part_num == 1 else f"{title}, part {part_num}", "text": "\n\n".join(current_paras)})
    ...
    final_segments = []
    for ch in batched:
        scenes = split_on_scenes(ch["title"], ch["text"])
        for sc in scenes:
            final_segments.extend(sub_split(sc["title"], sc["text"]))
