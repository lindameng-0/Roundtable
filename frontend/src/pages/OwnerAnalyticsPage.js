...
function Breakdown({ title, values, label, valueLabel = "Page views" }) {
  return <section><h2>{title}</h2>{Object.keys(values).length ? <table><thead><tr><th>{label}</th><th>{valueLabel}</th></tr></thead><tbody>{Object.entries(values).map(([key, value]) => <tr key={key}><td>{key}</td><td>{number(value)}</td></tr>)}</tbody></table> : <p>No recorded visits in this period.</p>}</section>;
}
...
export default function OwnerAnalyticsPage() {
  ...
  return <><SiteHeader /><main id="main-content" className="page-width owner-analytics" tabIndex={-1}>
    ...
    {data && <>
      ...
      <dl className="analytics-stats">{[["Public page views", data.pageviews], ["Signup-link clicks", data.signup_clicks], ["Distinct click visitors", data.unique_signup_click_visitors]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{number(value)}</dd></div>)}</dl>
      <section><h2>Signup funnel</h2><p>Events during the selected period. Accepted email signups have successfully sent a verification email; verified accounts completed the link.</p><table><thead><tr><th>Step</th><th>Events</th></tr></thead><tbody>{[["Signup page viewed", funnel.signup_pageviews], ["Signup link clicked", funnel.signup_clicks], ["Signup form submitted", funnel.signup_submissions], ["Verification email sent", funnel.signup_accepted], ["Verification email failed", funnel.signup_email_failed], ["Email verified", funnel.email_verified], ["Google auth started", funnel.google_auth_started], ["New Google account completed", funnel.google_signup_completed]].map(([label, value]) => <tr key={label}><td>{label}</td><td>{number(value)}</td></tr>)}</tbody></table><p className="small-note">Distinct submit visitors: {number(funnel.unique_signup_submit_visitors)}. AI referral page views: {number(aiViews)}.</p></section>
      ...
      <div className="analytics-columns"><Breakdown title="Where visits come from" label="Referrer category" values={data.sources} /><Breakdown title="What people read" label="Public page" values={data.pages} /><Breakdown title="Distinct signup click visitors" label="Originating page" valueLabel="Distinct visitors" values={data.unique_signup_clicks_by_path || {}} /></div>
      ...
      <p className="analytics-method">Page views and funnel events are counts, so repeats and bots can affect them. Distinct-visitor figures deduplicate a keyed hash of the request IP; Readerfold never stores or displays the raw IP. The pseudonymous hashes are retained for up to 90 days and cannot be compared after the secret is rotated. Aggregate counters are retained for up to 400 days. Referral categories come from the browser's incoming referrer; missing referrers appear as direct. Google AI and ordinary Google traffic cannot be separated here. No manuscript content, email addresses, raw referrers, query strings, or analytics cookies are stored. Do Not Track and Global Privacy Control requests are excluded.</p>
