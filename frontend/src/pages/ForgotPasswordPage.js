      ...
      <section className="w-full max-w-md border border-black/10 bg-[#fffdfa] p-8 shadow-[0_24px_70px_rgba(33,31,27,0.08)] sm:p-10">
        {sent ? (
          <div>
            ...
            <Link to="/login" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[#493449] hover:underline"><ArrowLeft className="h-4 w-4" />Return to sign in</Link>
          </div>
        ) : (
          <>
            <div className="mb-6 grid h-12 w-12 place-items-center rounded-full bg-[#f2e5e7] text-[#493449]"><Mail className="h-5 w-5" /></div>
            ...
            <form onSubmit={submit} className="mt-7">
              ...
              <button type="submit" disabled={submitting} className="mt-5 flex w-full items-center justify-center gap-2 bg-[#493449] px-4 py-3.5 text-sm font-semibold text-white hover:bg-[#624861] disabled:opacity-60">{submitting && <Loader2 className="h-4 w-4 animate-spin" />}Send reset link</button>
            </form>
