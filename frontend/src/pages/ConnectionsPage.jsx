...
export default function ConnectionsPage() {
  ...
  return <><SiteHeader /><main id="main-content" className="connections-page page-width" tabIndex={-1}>
    ...
    <p className="connections-intro">Let a compatible assistant work with your Readerfold manuscripts through MCP. Give each connection its own key so you can revoke it independently.</p>
    ...
    {!user?.email_verified ? <p role="status">Verify your account email before creating a connection.</p> : <>
      ...
      </div><div className="connection-instructions"><h2>In your assistant’s settings</h2><ol><li>Add a remote MCP server using Streamable HTTP.</li><li>Use this server URL:<code className="connection-code">{API}/mcp/</code></li><li>Add the header <code>Authorization</code>, with the value <code>Bearer YOUR_KEY</code>. Replace <code>YOUR_KEY</code> with the key you create here.</li><li>Connect and try: “List my Readerfold manuscripts.”</li></ol><p>Save the key in the client’s secure settings, not in a conversation, shared file, or URL. Connected assistants receive the writing and feedback they request. Review their data policies before connecting.</p></div></section>
      {secret && <section className="connection-secret" aria-labelledby="secret-heading"><h2 id="secret-heading">Save your key now</h2><p>This is the only time Readerfold will show the full key. It will disappear when you leave this page.</p><label className="sr-only" htmlFor="new-connection-key">New connection key</label><input ref={secretRef} id="new-connection-key" type={revealed ? "text" : "password"} value={secret} readOnly autoComplete="off" spellCheck={false} /><div className="connection-actions"><button className="button button-primary" onClick={copy}>Copy key</button><button className="button button-quiet" onClick={() => setRevealed(!revealed)}>{revealed ? "Hide key" : "Show key"}</button><button className="text-link" onClick={() => setSecret("")}>I’ve saved it</button></div></section>}
