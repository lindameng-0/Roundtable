...
const { buildStaticGuides } = require("./build-static-guides.cjs");
...
const socialImageAlt = siteConfig.socialImageAlt;
...
for (const [route, page] of Object.entries(pages)) {
  ...
  const head = `<title>${escape(page.title)}</title><meta name="description" content="${escape(page.description)}"><meta name="robots" content="index, follow, max-image-preview:large"><link rel="canonical" href="${url}"><meta property="og:title" content="${escape(page.title)}"><meta property="og:description" content="${escape(page.description)}"><meta property="og:url" content="${url}"><meta property="og:type" content="website"><meta property="og:site_name" content="Readerfold"><meta property="og:image" content="${socialImage}"><meta property="og:image:width" content="${siteConfig.socialImageWidth}"><meta property="og:image:height" content="${siteConfig.socialImageHeight}"><meta property="og:image:alt" content="${socialImageAlt}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(page.title)}"><meta name="twitter:description" content="${escape(page.description)}"><meta name="twitter:image" content="${socialImage}"><meta name="twitter:image:alt" content="${socialImageAlt}"><script id="search-structured-data" type="application/ld+json">${JSON.stringify(searchSchema(route)).replace(/</g, "\\u003c")}</script>`;
...
buildStaticGuides(build);
// The deployment domain is configured independently of the display name.
const siteUrl = new URL(siteConfig.origin);
if (siteUrl.protocol !== "https:" || siteUrl.origin !== siteConfig.origin) {
  throw new Error("siteConfig.origin must be an HTTPS origin without a path or trailing slash");
}
fs.writeFileSync(path.join(build, "CNAME"), siteUrl.hostname + "\n");
const robots = fs.readFileSync(path.join(root, "public/robots.txt"), "utf8");
fs.writeFileSync(path.join(build, "robots.txt"), robots.replace(/^Sitemap:.*$/m, `Sitemap: ${siteConfig.origin}/sitemap.xml`));
// SPA fallback is deliberately unindexed and never copies home-page metadata.
fs.writeFileSync(path.join(build, "404.html"), clean.replace("</head>", '<title>Readerfold workspace</title><meta name="robots" content="noindex, nofollow"></head>'));
fs.writeFileSync(path.join(build, "sitemap.xml"), `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${Object.entries({ ...pages, ...staticGuides }).map(([route, page]) => `<url><loc>${publicPageUrl(route)}</loc><lastmod>${page.lastModified}</lastmod></url>`).join("")}</urlset>`);
